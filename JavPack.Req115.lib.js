...
class Req115 extends Drive115 {
  ...
  static async listMagnet({ code, dir, regex, verifyOptions, rename, renameTxt, tags, clean, cover }, magnet) {
    const cid = await this.generateCid(dir);
    if (!cid) return { state: "error", msg: `${code} 获取目录失败` };

    const { url, zh, crack } = magnet; // 直接从单个磁力链接对象中解构
    ...
    if (!state) {
      res.state = "error";
      res.msg = `${code} ${error_msg}`;
      if (errcode === 10008) return res; // 如果需要继续处理其他逻辑，可以改为continue
      if (errcode === 911) {
        res.state = "warn";
        res.msg = error_msg;
      }
