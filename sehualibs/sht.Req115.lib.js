class Drive115 extends Req {
  ...
  /**
 * 添加多个离线任务
 * @param {string[]} urls - 链接数组
 * @param {string} wp_path_id - 保存路径 ID
 * @returns {Promise<{ success: string[], failed: { url: string, error: Error }[] }>} 返回成功和失败的结果
 */
  static lixianAddTaskUrls(urls, wp_path_id) {
    // 构建批量任务数据
    const data = { wp_path_id };
...
class Req115 extends Drive115 {
  ...
  static handleRename(files, cid, { rename, renameTxt, zh, crack, guochan, subs }) {
    ...
    rename = rename.replaceAll("$guochan", guochan ? renameTxt.guochan : "");
    rename = rename.trim();
